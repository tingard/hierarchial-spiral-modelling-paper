...
The inverse gamma distribution is used to aid the convergence of the Hamiltonian Monte Carlo algorithm used (discussed later). The Cauchy distirbution is equivalent to the Student's t-distribution with one degree of freedom, and was chosen due to its fatter tails than the normal distribution. Our likelihood function for $N$ arms, each of which with $n_\mathrm{arm}$ points is
